...
babylond tx authz grant $STAKER_CONTROLLER generic --msg-type "/cosmos.authz.v1beta1.MsgGrant" --expiration 0 \
  --from $STAKER --generate-only --offline -a 0 -s 0 > staker_tx_01.json
printf "."

babylond tx authz grant $STAKER_CONTROLLER generic --msg-type "/cosmos.authz.v1beta1.MsgRevoke" --expiration 0 \
  --from $STAKER --generate-only --offline -a 0 -s 0 > staker_tx_02.json
printf "."

babylond tx authz grant $STAKER_CONTROLLER generic --msg-type "/cosmos.feegrant.v1beta1.MsgGrantAllowance" --expiration 0 \
  --from $STAKER --generate-only --offline -a 0 -s 0 > staker_tx_03.json
printf "."

babylond tx authz grant $STAKER_CONTROLLER generic --msg-type "/cosmos.feegrant.v1beta1.MsgRevokeAllowance" --expiration 0 \
  --from $STAKER --generate-only --offline -a 0 -s 0 > staker_tx_04.json
printf "."

babylond tx authz grant $STAKER_CONTROLLER generic --msg-type "/cosmos.distribution.v1beta1.MsgSetWithdrawAddress" --expiration 0 \
...
babylond tx authz grant $GRANTEE generic --msg-type "/cosmos.distribution.v1beta1.MsgWithdrawDelegatorReward" --expiration 0 \
...
babylond tx distribution set-withdraw-addr $REWARDS_COLLECTOR \
