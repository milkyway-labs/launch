...
set -eu -o pipefail
echo -e "\033[92mStaker:\033[0m $STAKER"
...
echo
echo -e "\033[92mCreating staker_tx.json\033[0m"
...
initiad tx authz grant $GRANTEE generic --msg-type "/cosmos.staking.v1beta1.MsgDelegate" --expiration 0 \
  --from $STAKER --generate-only --offline -a 1 -s 0 > staker_tx_06.json
printf "."
...
echo
echo -e "\033[92mSuccessfully created staker_tx.json!\033[0m"

# Create rewards_collector_tx.json

echo
echo -e "\033[92mCreating rewards_collector_tx.json\033[0m"
...
echo
echo -e "\033[92mSuccessfully created rewards_collector_tx.json!\033[0m"